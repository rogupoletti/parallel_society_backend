...
// TODO: Replace with your actual old snapshot space ID
...
if (!admin.apps.length) {
    ...
    if (serviceAccountPath) {
        try {
            ...
            console.log(`� Using service account from: ${absolutePath}`);
            options.credential = admin.credential.cert(absolutePath);
        } catch (authError: any) {
            console.warn(`⚠️ Could not load service account from ${serviceAccountPath}: ${authError.message}`);
        }
    }
